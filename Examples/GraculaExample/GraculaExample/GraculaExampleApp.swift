...
struct GraculaExampleApp: App {
    private let compositionRoot = ExampleAppCompositionRoot()
...
struct ExampleAppCompositionRoot {
    @MainActor
...
private struct ExampleDemoPlanner: Planning {
    func makePlan(userText: String, context: ConversationContext) async throws -> AgentPlan {
        ...
        if lowercasedText.contains("app") {
            return AgentPlan(
                userText: userText,
                summary: "Open app",
                toolCalls: [
    ...
    private func extractAppName(from text: String) -> String {
        ...
        guard let markerIndex = tokens.firstIndex(where: { $0.lowercased() == "app" }),
